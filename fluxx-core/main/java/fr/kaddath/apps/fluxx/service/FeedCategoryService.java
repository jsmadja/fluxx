...
public class FeedCategoryService {
	...
	public List<String[]> findNumItemFeedsByCategory() {
		...
		Query q = em
				.createNativeQuery("SELECT fc.NAME, count(ifc.ITEM_ID) FROM FEEDCATEGORY fc, item_feedcategory ifc WHERE fc.name = ifc.FEEDCATEGORIES_NAME GROUP BY fc.name");
	...
	public List<String> findCategoryNamesInLowerCaseWithLike(String name, Integer numCategories) {
		Query q = em.createNativeQuery("SELECT distinct(lower(name)) FROM FEEDCATEGORY WHERE lower(name) like '%"
	...
	public List<String> findCategoryNamesWithLike(String name, Integer numCategories) {
		Query q = em.createNativeQuery("SELECT name FROM FEEDCATEGORY WHERE lower(name) like '%" + name
				+ "%' order by lower(name)");
		q.setMaxResults(numCategories);
		return q.getResultList();
	}
